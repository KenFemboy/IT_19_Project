    ...
    <div className="ttms-section space-y-6 pb-16">
      <section className="glass-card flex flex-col gap-4 p-6">
        ...
        <button
          ...
          className="flex h-24 items-center justify-center rounded-3xl bg-emerald-600 text-lg font-semibold text-white shadow-soft"
        >
          SOS
        </button>
      </section>
        ...
        <div className="glass-card p-6">
          ...
          <div className="mt-4 space-y-3">
            {emergencyHotlines.map((line) => (
              <div
                key={line.id}
                className="flex items-center justify-between rounded-2xl bg-emerald-50/70 p-3 text-xs text-emerald-800"
              >
      ...
      <section className="glass-card p-6">
        <p className="text-sm font-semibold text-slate-900">Barangay contacts</p>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {barangays.slice(0, 9).map((barangay) => (
            <div
              key={barangay}
              className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3 text-xs text-emerald-700"
            >
